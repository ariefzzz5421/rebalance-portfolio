...
function renderProfile(){
  ...
  const check='<span class="avatar-choice__check" aria-hidden="true"><svg viewBox="0 0 16 16" fill="none"><path d="m3.5 8 3 3 6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>';
  grid.innerHTML=window.PORSI_PROFILE.avatars.map(avatar=>`<button class="avatar-choice" type="button" data-avatar-id="${avatar.id}" aria-label="Avatar ${avatar.label}" aria-pressed="${profile.avatarId===avatar.id}"><span class="avatar-choice__media"><img src="${avatar.src}" alt="" width="52" height="52"></span><span class="avatar-choice__label">${avatar.label}</span>${check}</button>`).join('')+`<button class="avatar-choice avatar-choice--none" type="button" data-avatar-id="" aria-label="Tanpa avatar" aria-pressed="${!profile.avatarId}"><span class="avatar-choice__media"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="9" r="3"/><path d="M6.5 18c.7-2.3 2.5-3.5 5.5-3.5s4.8 1.2 5.5 3.5M4 4l16 16"/></svg></span><span class="avatar-choice__label">Tanpa</span>${check}</button>`;
  preview.textContent='';
