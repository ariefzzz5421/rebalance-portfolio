...
function drawSpark(canvas,points){
 ...
 data.forEach((p,i)=>{const x=pad+(i/(data.length-1))*(w-pad*2),y=h-pad-((p.v-min)/span)*(h-pad*2);if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y);});ctx.stroke();
}
function render(){
 grid.innerHTML=window.ASSETS.map(card).join('');filter();
 grid.querySelectorAll('.asset-card').forEach(el=>{const a=window.assetByTicker(el.dataset.ticker),intel=window.assetIntel(a),m=intel.marketSymbol?market[intel.marketSymbol]:null;drawSpark(el.querySelector('canvas'),m&&m.history);});
}
...
async function load(){
 ...
 try{const r=await fetch(`/api/market?symbols=${encodeURIComponent(symbols.join(','))}`);if(!r.ok)throw new Error('Market API unavailable');const j=await r.json();market=j.data||{};status.textContent='Market data loaded · cached up to 15 min';render();}
 catch(e){status.textContent='Market data unavailable — asset library tetap bisa dibuka';console.error(e);}
}
search.addEventListener('input',filter);window.addEventListener('resize',()=>render());load();
})();