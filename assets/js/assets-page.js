...
function render(){grid.innerHTML=window.ASSETS.map(card).join('');filter();grid.querySelectorAll('.asset-card').forEach(el=>{const a=window.assetByTicker(el.dataset.ticker),intel=window.assetIntel(a),m=intel.marketSymbol?market[intel.marketSymbol]:null;drawSpark(el.querySelector('canvas'),m&&m.history);});}
