(function(){
  ...
  function applyLanguage(){
    ...
    if(page==='portfolio'){
      ...
      assignText(document.querySelector('.page-head p'),'Set currency, language, and local data. Dark/light controls now sit directly below Settings in the sidebar.');
      ...
      const head=document.querySelector('.page-head p');if(head)assignText(head,'Track assets available in Porsi, their existing logos, historical charts, and 1M / 1Y / 5Y / 10Y annualized returns.');assignPlaceholder(document.getElementById('asset-search'),'Search BTC, BBCA, NVIDIA, gold…');
    }
  }
  ...
  function boot(){document.documentElement.lang=derive().language;applyLanguage();patchPortfolio();wrapExporter();if(document.body){let queued=false;const mo=new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;applyLanguage();patchPortfolio();});});mo.observe(document.body,{subtree:true,childList:true,characterData:true});}}
