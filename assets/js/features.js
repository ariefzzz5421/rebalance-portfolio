...
function chooseLabel(){return({id:'Pilih strategi',en:'Choose strategy',ja:'戦略を選択',zh:'选择策略'})[lang()]||'Choose strategy';}
...
function strategyInput(strategy){const assets=[],weights={};strategy.parts.forEach(([ticker,,weight])=>{const symbol=window.MARKET_SYMBOLS&&window.MARKET_SYMBOLS[ticker],entry=symbol&&strategyMarket[symbol];assets.push({ticker,history:entry&&Array.isArray(entry.totalReturnHistory)?entry.totalReturnHistory:[],availableFrom:entry&&entry.availableFrom,isStablecoin:ticker==='USDT',stablecoinFallbackUsed:!!(entry&&entry.stablecoinFallbackUsed)});weights[ticker]=weight/100;});return {assets,weights};}
...
function optionMarkup(key,s){return `<button class="strategy-option" type="button" role="option" data-strategy-option="${key}" aria-selected="${selected===key}">${strategyIcon(s)}<span class="strategy-option__copy"><strong>${s.title}</strong><small>${s.subtitle}</small><span class="strategy-option__metrics">${menuMetrics(key)}</span>${assetPreview(s)}</span><span class="strategy-option__arrow" aria-hidden="true">${selected===key?'✓':'↗'}</span></button>`;}
...
function updateTrigger(key){const label=$('#strategy-select-label'),meta=$('#strategy-select-cagr'),trigger=$('#strategy-select-trigger'),icon=trigger&&trigger.querySelector('.strategy-select__generic-icon');if(!label||!meta||!trigger)return;let preview=trigger.querySelector('.strategy-select__assets');if(!preview){preview=document.createElement('span');preview.className='strategy-select__assets';meta.after(preview);}if(!key){label.textContent=chooseLabel();meta.textContent='';preview.textContent='';if(icon)icon.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 18V9m5 9V5m5 13v-6m4 6V8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="m4 7 5-3 5 5 6-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';return;}const s=STRATEGIES[key];label.textContent=s.title;meta.textContent=compactPerformance(key);preview.innerHTML=assetPreview(s);if(icon)icon.innerHTML=strategyIcon(s);}
