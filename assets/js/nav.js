(function(){
  ...
  window.porsiRoute=path=>location.protocol==='file:'?(path==='/'?'index.html':`${path.slice(1)}.html`):path;
  ...
  const items=[
    ['portfolio','/','Portfolio','<path d="M4 19V9m5 10V5m5 14v-7m5 7V3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'],
    ['strategies','/strategies','Strategies','<path d="M12 16v5M16 14.639V21M20 10.656V21m22-18-8.646 8.646a.5.5 0 0 1-.708 0L9.354 8.354a.5.5 0 0 0-.707 0L2 15M4 18.463V21M8 14.656V21" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>'],
    ['currencies','/currencies','Kurs mata uang','<circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" stroke-width="2"/><path d="M15 8.5c-.6-.7-1.6-1.1-3-1.1-1.8 0-3 .9-3 2.3 0 1.3 1 1.9 3 2.3 2 .4 3 1 3 2.3 0 1.5-1.2 2.3-3 2.3-1.4 0-2.5-.4-3.2-1.2M12 5.5v13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>'],
    ...
    ['settings','/settings','Settings','<path d="M9.671 4.136a2.34 2.34 0 0 1 4.659 0 2.34 2.34 0 0 0 3.319 1.915 2.34 2.34 0 0 1 2.33 4.033 2.34 2.34 0 0 0 0 3.831 2.34 2.34 0 0 1-2.33 4.033 2.34 2.34 0 0 0-3.319 1.915 2.34 2.34 0 0 1-4.659 0 2.34 2.34 0 0 0-3.32-1.915 2.34 2.34 0 0 1-2.33-4.033 2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="2"/>']
  ];
  ...
  items.forEach((item,i)=>{if(i===4){const d=document.createElement('span');d.className='sidebar__divider';nav.appendChild(d);}const a=document.createElement('a');a.href=window.porsiRoute(item[1]);a.className='sidebar__link'+(page===item[0]?' is-active':'');a.title=item[2];a.setAttribute('aria-label',item[2]);a.innerHTML=`<svg viewBox="0 0 24 24" aria-hidden="true">${item[3]}</svg>`;nav.appendChild(a);});
  document.querySelectorAll('[data-porsi-route]').forEach(a=>{a.href=window.porsiRoute(a.dataset.porsiRoute);});
