...
function fetchOptions(){const o={headers:{'User-Agent':'Mozilla/5.0 Porsi/1.5','Accept':'application/json'}};if(typeof AbortSignal!=='undefined'&&AbortSignal.timeout)o.signal=AbortSignal.timeout(10000);return o;}
...
function parseResult(symbol,result,includeCagr){
  ...
  for(let i=0;i<ts.length;i++){const close=Number(quote[i]),adjusted=Number.isFinite(adj[i])?Number(adj[i]):close,t=ts[i]*1000;if(Number.isFinite(close)&&close>0)chartPoints.push({t,v:Number(close.toFixed(6))});if(Number.isFinite(adjusted)&&adjusted>0)returnPoints.push({t,v:Number(adjusted.toFixed(6))});}
  ...
  return {symbol,price:livePrice,previousClose:Number.isFinite(previousClose)?previousClose:null,currency:meta.currency||null,exchange:meta.exchangeName||null,timezone:meta.exchangeTimezoneName||null,asOf:new Date(endTime).toISOString(),source:'Yahoo Finance',history:chartPoints,cagr:includeCagr&&adjustedEnd?{m1:metric(returnPoints,adjustedEnd,1),y1:metric(returnPoints,adjustedEnd,12),y5:metric(returnPoints,adjustedEnd,60),y10:metric(returnPoints,adjustedEnd,120)}:{m1:null,y1:null,y5:null,y10:null}};
}
...
module.exports=async function handler(req,res){
  ...
  const raw=String(req.query.symbols||req.query.symbol||''),symbols=[...new Set(raw.split(',').map(s=>s.trim()).filter(Boolean))].slice(0,45),range=String(req.query.range||'').toUpperCase();
  ...
  try{
    ...
    const data=await fetchWithLimit(symbols,6);res.setHeader('Cache-Control','s-maxage=900, stale-while-revalidate=3600');return res.status(200).json({data,source:'Yahoo Finance historical chart API',method:'Summary CAGR uses adjusted close where available; history is monthly raw close.'});
